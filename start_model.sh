...
if [[ $# -lt 1 || -z "${1:-}" ]]; then
  echo "Użycie: $0 HOTKEY_ID[,HOTKEY_ID2,...] [ML_MAX_HANDS] [REMOVE_OTHER]"
  echo "Przykład: $0 214"
  echo "Przykład: $0 11,14,22 40 0"
  exit 1
fi
...
VENV_BIN="${POKER44_VENV_BIN:-$REPO/.venv-1/bin}"
...
if ! [[ "$ML_MAX_HANDS" =~ ^[0-9]+$ ]] || [[ "$ML_MAX_HANDS" -lt 1 ]]; then
