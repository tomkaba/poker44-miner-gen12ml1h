#!/usr/bin/env python3
"""Train improved single-hand model (v2) with hand-level feature engineering.

Why this script:
- Current 16-feature extractor was designed for multi-hand chunks and quickly saturates on 1-hand data.
- This script builds richer per-hand features and compares several model families.
- It also tunes decision threshold on validation split.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train improved single-hand model (v2)")
    parser.add_argument("--data-dir", type=Path, default=Path("data"))
    parser.add_argument(
        "--patterns",
        type=str,
        default="public_benchmark_1hand_1000_seed_*.json.gz,public_benchmark_1hand_1000_set2_seed_*.json.gz",
        help="Comma-separated glob patterns. By default uses both base and set2 datasets.",
    )
    ...
    parser.add_argument("--model-prefix", type=str, default="ml_single_hand_v2")
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--deduplicate", action="store_true")
    parser.add_argument("--optimize", choices=["accuracy", "f1", "balanced_accuracy"], default="f1")
    parser.add_argument("--thresholds", type=str, default="0.30,0.35,0.40,0.45,0.50,0.55,0.60,0.65,0.70")
...
def parse_pattern_list(text: str) -> List[str]:
    patterns = [tok.strip() for tok in text.split(",") if tok.strip()]
    if not patterns:
...
def _bb(hand: Dict[str, Any]) -> float:
    bb = safe_float((hand.get("metadata") or {}).get("bb"))
...
def extract_xy(rows: Sequence[Dict[str, Any]], split_name: str) -> Tuple[np.ndarray, np.ndarray]:
    ...
    y_list: List[int] = []
    log(f"Extracting v2 features for {split_name}: {len(rows)} rows")
...
def main() -> None:
    ...
    patterns = parse_pattern_list(args.patterns)

    data_dir = args.data_dir.expanduser().resolve()
    weights_dir = args.weights_dir.expanduser().resolve()
    weights_dir.mkdir(parents=True, exist_ok=True)

    log("Starting single-hand v2 training")
    log(f"Optimize metric: {args.optimize}")
    ...
    for pattern in patterns:
        ...
        log(f"Pattern '{pattern}' matched files: {len(matched)}")
        for p in matched:
            all_files_set.add(p)

    all_files = sorted(all_files_set)
    if not all_files:
        raise FileNotFoundError(f"No files matched any pattern in {patterns} under {data_dir}")
    ...
    log(f"Evaluating {len(models)} model families")
    for idx, (name, model) in enumerate(models.items(), 1):
        ...
        log(
            f"  best threshold={best_thr:.3f} acc={best_metrics['accuracy']:.4f} "
            f"f1={best_metrics['f1']:.4f} bal_acc={best_metrics['balanced_accuracy']:.4f}"
        )
    ...
    final_model = candidate_models(args.seed)[best["name"]]
    ...
    # Compatibility with existing loader path contract.
    ...
    print()
    print("=== SINGLE-HAND V2 TRAINING COMPLETE ===")
